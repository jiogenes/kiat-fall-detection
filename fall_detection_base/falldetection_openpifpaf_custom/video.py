...
### MODIFIED ###
import numpy as np
from .custom_models import FallDetector
from .custom_data import preprocess_keypoints_batch
################
...
def inference(args, stream):
    ...
    for frame_i, (ax, ax_second) in enumerate(animation.iter()):
        ...
        ### MODIFIED ###
        keypoints_dict["keypoints"].append([pred.data.tolist() for pred in preds])
        keypoints_dict["skeleton"].append([[list(sk) for sk in pred.skeleton] for pred in preds])
        ################
        ...
        ### MODIFIED ###
        if fallcount is not None and old_fallcount != fallcount:
            fall_predictions.append(frame_i)
        ################
    ...
    ### MODIFIED ###
    if os.path.exists("fall_detector.pt"):
        fall_detector = FallDetector()
        fall_detector.load_state_dict(torch.load("fall_detector.pt", map_location="cpu"))

        keypoints = np.array([kpts[0] for kpts in keypoints_dict["keypoints"] if len(kpts) > 0])
        keypoints += np.random.normal(loc=0, scale=(keypoints.max() - keypoints.min())/100, size=keypoints.shape)
        skeleton = np.array([sks[0] for sks in keypoints_dict["skeleton"] if len(sks) > 0]) - 1
        keypoints = preprocess_keypoints_batch(keypoints, skeleton)
        keypoints = torch.tensor(keypoints, dtype=torch.float32)[None]
        with torch.no_grad():
            pred = fall_detector(keypoints)
        pred = (pred >= 0.5).int().cpu().numpy().tolist()[0]
    ################
    ...
    ### MODIFIED ###
    name = os.path.splitext(RTSPURL.split("/")[-1])[0]
    with open(os.path.join(os.path.dirname(args.video_output), f"prediction_{name}.txt"), "w") as f:
        f.write("\n".join(map(str, fall_predictions)))

    if os.path.exists("fall_detector.pt"):
        with open(os.path.join(os.path.dirname(args.video_output), f"prediction_final_{name}.txt"), "w") as f:
            f.write(f"{pred}")

    with open(os.path.join(os.path.dirname(args.video_output), f"keypoints_{name}.json"), "w") as f:
